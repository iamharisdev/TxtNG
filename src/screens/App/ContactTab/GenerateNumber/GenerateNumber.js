...
const GenerateNumber = ({navigation}) => {
  const {colors} = useTheme();
  const phoneListRef = useRef(null);
  const {phone_list} = useSelector(state => state.auth);
  const {checkout_detail} = useSelector(state => state?.appReducer);
  const [loading, setLoading] = useState(false);
  const [phone, setphone] = useState({text: checkout_detail?.phone});
  ...
  const onConfirmNumber = async () => {
    ...
    if (check) {
      try {
        ...
        if (res?.data) {
          checkout_detail.phone = res?.data;
          dispatch(
            current_checkout_request(checkout_detail, res => {
              setLoading(false);
